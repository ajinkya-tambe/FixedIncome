...
import { TrendingUp, TrendingDown, DollarSign, PieChart, RefreshCw, AlertCircle, Target } from "lucide-react"
...
export function NetPositionWindow() {
  ...
  const [calculationMethod, setCalculationMethod] = useState<"FIFO" | "LIFO" | "WAP">("WAP")
  ...
  const fetchData = async () => {
    try {
      setLoading(true)
      const [bondsData, positionsData] = await Promise.all([api.getBonds(), api.getPositions()])
      setBonds(bondsData)
  ...
  const calculatePortfolioMetrics = () => {
    ...
    positions.forEach((position) => {
      const bond = bonds.find((b) => b.ticker === position.ticker || b.id === position.bondId)
      if (!bond) return
      ...
      sectorMap.set(bond.sector, {
        ...
        pnl: existing.pnl + (position.totalPnL || position.unrealizedPnL + position.realizedPnL),
      })
    })
    ...
    setPortfolioMetrics({
      ...
      weightedAvgDuration: totalValue > 0 ? weightedDurationSum / totalValue : 0,
      totalPositions: positions.length,
    })

    // Calculate sector allocations
    const allocations: SectorAllocation[] = Array.from(sectorMap.entries()).map(([sector, data]) => ({
      sector,
      value: data.value,
  ...
  return (
    <div className="space-y-6">
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className={`text-sm flex items-center ${getPnLColor(portfolioMetrics.dayChange)}`}>
                  {getPnLIcon(portfolioMetrics.dayChange)}
                  <span className="ml-1">
                    {formatCurrency(portfolioMetrics.dayChange)} ({formatPercentage(portfolioMetrics.dayChangePercent)})
                  </span>
                </div>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className={`text-2xl font-bold ${getPnLColor(portfolioMetrics.totalPnL)}`}>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <div className="text-2xl font-bold text-purple-400">{portfolioMetrics.totalPositions}</div>
      ...
      <Card className="bg-slate-900 border-slate-700">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-xl text-blue-400">Net Position Analysis</CardTitle>
        ...
        <CardContent>
          <Tabs defaultValue="positions" className="w-full">
            <TabsList className="grid w-full grid-cols-3 bg-slate-800 border-slate-700">
              ...
              <TabsTrigger value="sectors" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white">
                Sector Allocation
              </TabsTrigger>
              ...
              <div className="space-y-4">
                {positions.length === 0 ? (
                  ...
                  positions.map((position) => {
                    ...
                    return (
                      <Card key={position.ticker || position.bondId} className="bg-slate-800 border-slate-600">
                        <CardContent className="p-6">
                          <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-3">
                              <div>
                                <div className="font-semibold text-white text-lg">{bond.ticker}</div>
                                <div className="text-slate-400 text-sm">{bond.issuer}</div>
                              </div>
                              <Badge className="bg-blue-600 text-white">{bond.sector}</Badge>
                              <Badge className="bg-green-600 text-white">{bond.rating}</Badge>
                            </div>
                            <div className="text-right">
                              <div
                                className={`text-lg font-bold ${getPnLColor(position.totalPnL || position.unrealizedPnL + position.realizedPnL)}`}
                              >
                                {formatCurrency(position.totalPnL || position.unrealizedPnL + position.realizedPnL)}
                              </div>
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-4">
                            <div>
                              ...
                              <div className="text-white font-semibold">{position.quantity.toLocaleString()}</div>
                            </div>
                            <div>
                              <span className="text-slate-400 text-sm">Avg Price:</span>
                              <div className="text-white font-semibold">${position.averagePrice.toFixed(2)}</div>
                            </div>
                            <div>
                              <span className="text-slate-400 text-sm">Current Price:</span>
                              <div className="text-white font-semibold">
                                ${(position.currentPrice || bond.currentPrice).toFixed(2)}
                              </div>
                            </div>
                            <div>
                            ...
                            <div>
                              ...
                              <div className={`font-semibold ${getPnLColor(position.realizedPnL)}`}>
                            ...
                            <div>
                              ...
                              <span className="text-white ml-2 font-semibold">
                                {portfolioMetrics.totalValue > 0
                                  ? formatPercentage((position.marketValue / portfolioMetrics.totalValue) * 100)
                                  : "0.00%"}
                              </span>
                            </div>
                          </div>
            ...
            <TabsContent value="sectors" className="mt-6">
              <div className="space-y-4">
                {sectorAllocations.map((allocation) => (
                  <Card key={allocation.sector} className="bg-slate-800 border-slate-600">
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <div className="font-semibold text-white">{allocation.sector}</div>
                          <Badge className="bg-blue-600 text-white">{formatPercentage(allocation.percentage)}</Badge>
                        </div>
                        <div className={`font-semibold ${getPnLColor(allocation.pnl)}`}>
                          {formatCurrency(allocation.pnl)}
                        </div>
                      </div>
                      <div className="flex items-center justify-between text-sm text-slate-400 mb-2">
                        <span>Market Value: {formatCurrency(allocation.value)}</span>
                        <span>{formatPercentage(allocation.percentage)} of portfolio</span>
                      </div>
                      <Progress value={allocation.percentage} className="h-2 bg-slate-700" />
                    </CardContent>
                  </Card>
                ))}
              </div>
            ...
            <TabsContent value="risk" className="mt-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card className="bg-slate-800 border-slate-600">
                  <CardHeader>
                    <CardTitle className="text-lg text-blue-400">Duration Risk</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-white font-semibold">
                          {portfolioMetrics.weightedAvgDuration.toFixed(2)} years
                        </span>
                      </div>
                      <div className="flex justify-between items-center">
                      ...
                      <div className="text-xs text-slate-500">
                        ...
                        1% rate change ≈{" "}
                        {(portfolioMetrics.weightedAvgDuration * portfolioMetrics.totalValue * 0.01).toFixed(0)} impact
                      </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card className="bg-slate-800 border-slate-600">
                  <CardHeader>
                    <CardTitle className="text-lg text-blue-400">Yield Analysis</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-green-400 font-semibold">
                          {formatCurrency((portfolioMetrics.totalValue * portfolioMetrics.weightedAvgYield) / 100)}
                        </span>
                      </div>
                      <div className="text-xs text-slate-500">
                        <AlertCircle className="inline h-3 w-3 mr-1" />
