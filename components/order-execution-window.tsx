...
export function OrderExecutionWindow() {
  ...
  const [orderMethod, setOrderMethod] = useState<"MARKET" | "LIMIT" | "STOP" | "CONDITIONAL">("MARKET")
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      const orderData = {
        ...
        ...(useDisclosedQty && disclosedQuantity && { disclosedQuantity: Number.parseFloat(disclosedQuantity) }),
        ...(useStopLoss && stopLoss && { stopLoss: Number.parseFloat(stopLoss) }),
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-6">
        <Card className="bg-slate-900 border-slate-700">
          <CardHeader>
            <CardTitle className="text-xl text-blue-400">New Order Entry</CardTitle>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Select onValueChange={handleBondSelect}>
                ...
                <SelectContent className="bg-slate-800 border-slate-600">
                  {(orderType === "SELL" ? getAvailableBondsForSell() : bonds).map((bond) => {
                        ...
                        <div className="flex items-center justify-between w-full">
                          <span>
                            ...
                            {orderType === "SELL" && position && (
                              <span className="text-green-400 ml-2">({position.quantity} available)</span>
                            )}
                          </span>
                          <span className="text-slate-400 ml-4">${bond.currentPrice.toFixed(2)}</span>
                        </div>
                      </SelectItem>
                    )
                  })}
                ...
                <CardContent className="p-4">
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <span className="text-white ml-2 font-semibold">${selectedBond.currentPrice.toFixed(2)}</span>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <span className="text-green-400 ml-2">${selectedBond.bidPrice.toFixed(2)}</span>
                    </div>
                    <div>
                      <span className="text-slate-400">Ask:</span>
                      <span className="text-red-400 ml-2">${selectedBond.askPrice.toFixed(2)}</span>
                    </div>
                    {orderType === "SELL" && (
                      <>
                        <div>
                        ...
                        <div>
                          ...
                          <span className="text-white ml-2">
                            ${getPositionDetails(selectedBond.id)?.averagePrice.toFixed(2) || "0.00"}
                          </span>
                        </div>
                      </>
                    )}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={orderMethod} onValueChange={(value: any) => setOrderMethod(value)}>
                  <SelectTrigger className="bg-slate-800 border-slate-600 text-white">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="bg-slate-800 border-slate-600">
              ...
              <div className="space-y-2">
                <Label className="text-slate-300">
                  ...
                  {orderType === "SELL" && selectedBond && (
                    <span className="text-xs text-slate-400 ml-2">(Max: {getAvailableQuantity(selectedBond.id)})</span>
                  )}
                </Label>
                <Input
                  type="number"
                  ...
                  max={orderType === "SELL" && selectedBond ? getAvailableQuantity(selectedBond.id) : undefined}
                />
              </div>
            ...
            {quantity && price && (
              <div className="bg-slate-800 p-3 rounded border border-slate-600">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-white font-semibold text-lg">${calculateOrderValue().toLocaleString()}</span>
                </div>
                {orderType === "SELL" && selectedBond && (
                  <div className="flex justify-between items-center mt-2">
                    ...
                    <span
                      className={`font-semibold ${
                        (Number.parseFloat(price) - (getPositionDetails(selectedBond.id)?.averagePrice || 0)) >= 0
                          ? "text-green-400"
                          : "text-red-400"
                      }`}
                    >
                      $
                      {(
                        (Number.parseFloat(price) - (getPositionDetails(selectedBond.id)?.averagePrice || 0)) *
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-semibold text-slate-300">Advanced Options</h3>

              {/* Disclosed Quantity */}
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  <Label className="text-slate-300">Disclosed Quantity</Label>
                  <p className="text-xs text-slate-500">Only show partial quantity to market</p>
                </div>
                <Switch checked={useDisclosedQty} onCheckedChange={setUseDisclosedQty} />
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-500">Automatic sell trigger price</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-500">Execute based on market conditions</p>
                </div>
                <Switch checked={useConditional} onCheckedChange={setUseConditional} />
              </div>
      ...
      <div className="space-y-6">
        <Card className="bg-slate-900 border-slate-700">
          <CardHeader>
            <CardTitle className="text-xl text-blue-400">Recent Orders</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {orders.length === 0 ? (
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            ...
                            <Badge className={`${getOrderStatusColor(order.status)} text-white`}>{order.status}</Badge>
                            <Badge
                              variant="outline"
                              className={`${
                                order.type === "BUY" ? "border-green-500 text-green-400" : "border-red-500 text-red-400"
                              }`}
                            >
                              {order.type}
                            </Badge>
                        ...
                        <div className="grid grid-cols-2 gap-4 text-sm">
                          <div>
                            ...
                            <span className="text-white ml-2">{bond?.ticker || order.bondId}</span>
                          </div>
                          <div>
                            <span className="text-slate-400">Quantity:</span>
                            <span className="text-white ml-2">{order.quantity.toLocaleString()}</span>
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <span className="text-white ml-2">{order.orderType}</span>
                          </div>
                        </div>
